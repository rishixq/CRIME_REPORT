...
export function LocationInput({
  ...
  // Function to fetch geolocation data from MapTiler
  const fetchLocationData = async (query: string) => {
    const accessToken = process.env.NEXT_PUBLIC_MAPTILER_ACCESS_TOKEN;
    if (!accessToken) {
      console.error("MapTiler API key is missing");
      return;
    }

    const url = `https://api.maptiler.com/geocoding/${query}.json?key=${accessToken}`;
    ...
    try {
      ...
      if (data.features && data.features.length > 0) {
        const { center } = data.features[0]; // First result (best match)
        if (onCoordinatesChange) {
          onCoordinatesChange(center[1], center[0]); // lat, lng
        }
        onChange(query); // Update the input field with the query
      } else {
        setLocationError("No location found");
      }
    } catch (error) {
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        <input
          ...
          onBlur={() => fetchLocationData(value)} // Trigger geocoding when input loses focus
